 ...
 *  - setMaxFileSize(?int $bytes): void
  - setDirPermissions(int $permissions): void
 */
final class ArchiveUtility
{
    ...
    public static function validateArchive(string $path): bool
    {
        ...
        try {
            switch ($ext) {
                ...
                case 'zip':
                    return self::validateZipArchive($path);
    ...
    /**
     * Validate a ZIP archive, treating password-protected files as valid.
     */
    private static function validateZipArchive(string $path): bool
    {
        ...
        if ($status === true) {
            $valid = $zip->numFiles >= 1;
            $zip->close();
            return $valid;
        }

        if (in_array($status, [ZipArchive::ER_NOPASSWD, ZipArchive::ER_WRONGPASSWD, ZipArchive::ER_ENCRNOTSUPP], true)) {
            // Cannot fully validate without password, but file exists and is encrypted
            return true;
        }
    ...
    public static function isPasswordProtected(string $path): bool
    {
        ...
        if ($status !== true) {
            return in_array(
                $status,
                [ZipArchive::ER_NOPASSWD, ZipArchive::ER_WRONGPASSWD, ZipArchive::ER_ENCRNOTSUPP],
                true
            );
        }
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if ($stat !== false && isset($stat['encryption_method']) && (int) $stat['encryption_method'] !== 0) {
                $isProtected = true;
                break;
            }
        }
     ...
     *
     * @param int $threads Thread count (0/-1 = auto-detect all cores)
    ...
    private static function validatePath(string $path): void
    {
        ...
        if (str_starts_with($path, DIRECTORY_SEPARATOR) || preg_match('/^[A-Za-z]:[\\\\\\/]/', $path) === 1) {
            throw new RuntimeException("Path uses absolute location: $path");
        }
    }
    ...
    /**
     * Execute a command with file input streaming to stdout, captured to output file.
     * Memory-efficient for large files.
     *
     * @param array<string> $args Command and arguments
    ...
    private static function runCommandFileToFile(array $args, string $inputFile, string $outputFile): void
    {
        ...
        try {
            ...
            $process->run(function ($type, $buffer) use ($outputHandle): void {
                if ($type === Process::OUT) {
    ...
    private static function compressWithZstd(string $src, string $dst): void
    {
        ...
        $threads = self::$zstdThreads;
        $threadFlag = '-T' . ($threads < 0 ? 0 : $threads);
    ...
    private static function decompressZip(string $src, string $dstDir): string
    {
        ...
        $name = self::firstZipFileName($zip);
        ...
        $destination = rtrim($dstDir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . basename($name);
        if (@file_put_contents($destination, $content) === false) {
            throw new RuntimeException("Failed to write decompressed output: $destination");
        }
    ...
    /**
     * Return the first non-directory file entry in a ZIP archive.
     *
     * @throws RuntimeException If no file entries exist
     */
    private static function firstZipFileName(ZipArchive $zip): string
    {
        if ($zip->numFiles < 1) {
            throw new RuntimeException('Zip archive is empty');
        }
        ...
        throw new RuntimeException('Zip archive contains no files');
    }
    ...
    /**
     * Delete a file if it exists; ignore failures.
     */
